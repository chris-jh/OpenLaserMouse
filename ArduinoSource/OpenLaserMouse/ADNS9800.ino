...
extern const unsigned short firmware_length = 3070;
...
boolean adns_upload_firmware(){
  // send the firmware to the chip, cf p.18 of the datasheet
  Serial.println("Uploading firmware...");
  //adns_write_reg(REG_Configuration_I, 0x01); // 200cpi
...
volatile int* readADNS(){
  digitalWrite(PIN_NCS,LOW);
  //delta_x = (int) adns_read_reg(REG_Delta_X_L) | ((int) adns_read_reg(REG_Delta_X_H)<<8);
  //delta_y = (int) adns_read_reg(REG_Delta_Y_L) | ((int) adns_read_reg(REG_Delta_Y_H)<<8);
  xydat[0] = adns_read_reg(REG_Motion);
  xydat[1] = (int)adns_read_reg(REG_Delta_X_L);
  xydat[2] = (int)adns_read_reg(REG_Delta_X_H);
  xydat[3] = (int)adns_read_reg(REG_Delta_Y_L);
  xydat[4] = (int)adns_read_reg(REG_Delta_Y_H);
  digitalWrite(PIN_NCS,HIGH);
  return xydat;
}
